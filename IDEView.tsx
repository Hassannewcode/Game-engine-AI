...
const IDEView: React.FC<IDEViewProps> = ({ workspaceType, generatedCode, chatHistory, isLoading, onGenerate, onReset, onPositiveFeedback }) => {
        ...
        <div className="flex h-screen w-screen bg-black text-gray-300 font-sans">
            {/* Chat Panel (Left) */}
             <div className={`bg-[#121212] flex flex-col flex-shrink-0 border-r border-gray-800/70 transition-all duration-300 ease-in-out ${isChatVisible ? 'w-[380px]' : 'w-0'}`}>
                {isChatVisible && <ChatPanel history={chatHistory} onSend={onGenerate} isLoading={isLoading} onReset={onReset} onPositiveFeedback={onPositiveFeedback} />}
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <main className="flex-1 bg-black flex flex-row overflow-hidden">
                    {/* Code Pane */}
                    <div className={`h-full overflow-auto transition-all duration-300 ease-in-out font-mono text-sm ${isPreviewVisible ? 'w-1/2 border-r border-gray-800/70' : 'w-full'}`}>
                         <div className="p-4 h-full">
                            <pre><code className="language-html">{generatedCode}</code></pre>
                        </div>
                    </div>
