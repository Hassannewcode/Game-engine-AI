...
const ChatPanel: React.FC<ChatPanelProps> = ({ history, isLoading, onSend, onReset, onPositiveFeedback }) => {
                    ...
                    {history.map((msg) => (
                        <div key={msg.id} className={`flex gap-3 items-start ${msg.role === 'user' ? 'justify-end' : ''}`}>
                            ...
                            <div className="group relative">
                                <div className={`text-sm leading-relaxed rounded-lg px-4 py-2 max-w-sm ${msg.role === 'model' ? 'bg-gray-800/50 text-gray-300' : 'bg-blue-600/80 text-white'}`}>
                                    {msg.text}
                                </div>
                                {msg.role === 'model' && !msg.rated && (
                                     <button 
